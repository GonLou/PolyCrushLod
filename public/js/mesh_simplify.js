...
THREE.simplify_modifier = function simplify_modifier() {

}

THREE.simplify_modifier.prototype.modify =

(function() {
	function symetric_matrix() {
		// init
		this.m = new Array(10).fill(0);
	}


	symetric_matrix.prototype = {
    // defining matrix 
		set: function set(
			m11, m12, m13, m14,
			m22, m23, m24,
		...
		make_plane: function make_plane( a, b, c, d ) {
      // VELHOTE
			return this.set(
				a * a, a * b, a * c, a * d,
				       b * b, b * c, b * d,
      ...
      var det = 
		...
		// produces new Matrix
		add: function add( n ) {
			return new symetric_matrix().set(
				this.m[0] + n.m[0],
				this.m[1] + n.m[1],
				this.m[2] + n.m[2],
				this.m[3] + n.m[3],
	...
	function simplify_mesh(target_count, agressiveness) {
		...
		var deleted0          = []
    var deleted1          = []; // std::vector<int>
		var triangle_count    = triangles.length;


    conversion = this.conversion
		for (var iteration = 0; iteration < conversion; iteration++ ) {
			...
			//
			var threshold = 0.000000000001 * Math.pow( iteration + 3, agressiveness );
	...
	function update_mesh( iteration ) /*int*/
	{
		...
		if( iteration === 0 ) {
			for (var i = 0; i < vertices.length; i++ ) {
				...
				vertices[i].q = new symetric_matrix();
			}
			...
			for (var i = 0; i < triangles.length; i++) {
				...
				t.n = n;
				var tmp = new symetric_matrix().make_plane(
						n.x,
						n.y,
						n.z,
						-n.dot(p[0]))
				...
				// vertices[t.v[j]].q =
				// vertices[t.v[j]].q.add(symetric_matrix(n.x,n.y,n.z,-n.dot(p[0])));
			}
	...
	function vertex_error(/*symetric_matrix*/ q, /*double*/ x, y, z) {
    ...
    var div = document.getElementById('log');
		...
		div.innerHTML += '<br/>Old vertices ' + geometry.vertices.length, 'old faces ' + geometry.faces.length;

		div.innerHTML += '<br/>New vertices ' + vertices.length, 'old faces ' + triangles.length;
