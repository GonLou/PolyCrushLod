...
function InitSimplification(mesh_in)
{
    mesh = mesh_in;
...
function ComputeOptimalCoordAndCost(ei)
{
	...
	var v0 = ei.halfedge[0].vertex;
    var v1 = ei.halfedge[0].next.vertex;
    ...
    var matrix[4][4], rhs[4] = { 0.0, 0.0, 0.0, 1.0 }, solution[4];
    ...
    if( SolveLinearSystem(matrix, rhs, solution) ){
        ...
        if(v0.isBoundary) 
			for(int i = 0; i < 3; i++) 
				optimalCoord[i] = v0.coord[i];
        else               
			for(int i = 0; i < 3; i++) 
				optimalCoord[i] = v1.coord[i];
    }
...
function RemoveEdge(ei, optimalCoord, isFirstCollapse)
{
    ...
    var hepCollapse = ei.halfedge[0];

	//VertexIter v0 = hepCollapse.vertex;
    var v0 = hepCollapse.vertex;
    var v1 = hepCollapse.next.vertex;
    ...
    for(int i = 0; i < 3; i++) 
		vertexSplitTarget.top().v1OrginalCoord[i] = v1.coord[i];
    ...
    var facesOriginallyIncidentToV0OrV1; 

    if(v0.isBoundary == false) 
		startHalfEdge = hepCollapse;
    else                        
		startHalfEdge = FindBoundaryEdgeIncidentToVertexInCW(hepCollapse);

	//HalfEdge *hep = startHalfEdge;
    var hep = startHalfEdge;
    do{
...
//#endif
    ...
    if(hepCollapse.next.mate != NULL) 
		hepCollapse.next.mate.mate = hepCollapse.prev.mate;
    if(hepCollapse.prev.mate != NULL) 
		hepCollapse.prev.mate.mate = hepCollapse.next.mate;
    ...
    if(hepCollapse.prev.mate != NULL) 
		hepCollapse.prev.mate.edge = hepCollapse.next.edge;


    if(hepCollapse.mate != NULL){ // when "ect.ei" is not a boundary edge
        if(hepCollapse.mate.next.mate != NULL) 
			hepCollapse.mate.next.mate.mate = hepCollapse.mate.prev.mate;
        if(hepCollapse.mate.prev.mate != NULL) 
			hepCollapse.mate.prev.mate.mate = hepCollapse.mate.next.mate;
   ...
   if( hepCollapse.next.edge.isActive == false && 
    ...
    do{
       ...
       if(isFirstCollapse) 
		   ComputeOptimalCoordAndCost(hep.edge);
        ...
        if(hep.prev.mate == NULL){
            if(isFirstCollapse) 
				ComputeOptimalCoordAndCost(hep.prev.edge);    
...
var FindBoundaryEdgeIncidentToVertexInCW(baseHalfEdge)
{
    ///////////////////////////////////////////////////////////////
...
function IsFinWillNotBeCreated(ei)
{
	//HalfEdge *hepCollapse = ei.halfedge[0];
    var hepCollapse = ei.halfedge[0];
    ...
    var startHalfEdgeV0, startHalfEdgeV1;

    // change neighborHE to make sure that it is outside of collapsed face
    if(v0.isBoundary == false) 
		startHalfEdgeV0 = hepCollapse;
    else                        
		startHalfEdgeV0 = FindBoundaryEdgeIncidentToVertexInCW(hepCollapse);


    if(v1.isBoundary == false) 
		startHalfEdgeV1 = hepCollapse.next;
    else                        
		startHalfEdgeV1 = FindBoundaryEdgeIncidentToVertexInCW(hepCollapse.next);

	// HalfEdge *hepV0 = startHalfEdgeV0;
    var hepV0 = startHalfEdgeV0;
    do{
        var hepV1 = startHalfEdgeV1;
        do{
...
function VertexSplit()
{
    if(vertexSplitTarget.empty() == false){
        ...
        var hepCollapsed = vertexSplitTarget.top().ei.halfedge[0];

		//VertexIter v0 = hepCollapsed.vertex; 
        var v0 = hepCollapsed.vertex; 
        var v1 = hepCollapsed.next.vertex; 
        ...
        for(int i = 0; i < 3; i++) 
			v1.coord[i] = vertexSplitTarget.top().v1OrginalCoord[i];
        ...
        for(unsigned int i = 0; i < vertexSplitTarget.top().halfedgesAroundV0.size(); i++){
            var hep = vertexSplitTarget.top().halfedgesAroundV0[i];
...
function ControlLevelOfDetail(step)
{
    var n_target_faces = mesh.n_faces*pow(0.95, step);
